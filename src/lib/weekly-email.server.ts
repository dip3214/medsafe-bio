// Weekly MedSafe check-in email — sent Monday morning ("let's be healthy this week")
// and Saturday morning ("how did the week feel?"). Uses Resend directly with a
// verified sender so the From line reads as MedSafe.

import { supabaseAdmin } from "@/integrations/supabase/client.server";

type Cadence = "monday" | "saturday";

const FROM = process.env.MEDSAFE_EMAIL_FROM || "MedSafe <hello@med-safe.live>";
const APP_URL = process.env.PUBLIC_APP_URL || "https://med-safe.live";

export async function sendWeeklyCheckInEmails(cadence: Cadence) {
  const RESEND_API_KEY = process.env.RESEND_API_KEY;
  if (!RESEND_API_KEY) throw new Error("RESEND_API_KEY not configured");

  // Only users who opted in and haven't already been sent to today.
  ...
  const recipients: { user_id: string; email: string; name: string | null; token: string }[] = [];
  ...
  for (const r of recipients) {
    const html = renderEmail(cadence, r.name, r.token);
    ...
    try {
      const res = await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          html,
          headers: {
            "List-Unsubscribe": `<${APP_URL}/unsubscribe?token=${r.token}>`,
          },
        }),
      });
      if (!res.ok) {
        const t = await res.text();
        results.push({ email: r.email, ok: false, err: `${res.status} ${t}` });
        continue;
      }
      await supabaseAdmin
        .from("email_preferences")
      ...
      results.push({ email: r.email, ok: false, err: String(e?.message ?? e) });
    }
  }

  return { cadence, total: recipients.length, sent: results.filter((x) => x.ok).length, results };
}
        ...
        <tr><td style="padding:24px 28px 8px;border-bottom:1px solid #eee">
          <table role="presentation"><tr>
            <td style="vertical-align:middle">
              <div style="width:36px;height:36px;border-radius:50%;background:${brand};display:inline-block;text-align:center;line-height:36px;color:#fff;font-size:18px">♥</div>
            </td>
